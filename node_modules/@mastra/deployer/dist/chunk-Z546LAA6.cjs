    ...
    resolveId(id, importer) {
      ...
      try {
        ...
        for (const ext of [".mjs", ".js", ".cjs"]) {
          ...
          if (resolved2) {
            const pkgName = chunkIAEJ3C3J_cjs.getPackageName(id);
            if (!pkgName) {
              return null;
            }
...
async function getInputOptions2(entryFile, platform, env, { sourcemap = false } = {}) {
  ...
  const projectRoot = closestPkgJson ? path.dirname(chunkIAEJ3C3J_cjs.slash(closestPkgJson)) : chunkIAEJ3C3J_cjs.slash(process.cwd());
  const { workspaceMap, workspaceRoot } = await chunkIAEJ3C3J_cjs.getWorkspaceInformation({ mastraEntryFile: entryFile });
  const analyzeEntryResult = await chunkIAEJ3C3J_cjs.analyzeBundle(
    [entryFile],
    entryFile,
    {
  ...
  for (const [dep, metadata] of analyzeEntryResult.dependencies.entries()) {
    const pkgName = chunkIAEJ3C3J_cjs.getPackageName(dep);
    if (pkgName && workspaceMap.has(pkgName)) {
      deps.set(dep, metadata);
    }
  }
  const inputOptions = await chunkNSINCI76_cjs.getInputOptions(
    entryFile,
    {
      dependencies: deps,
  ...
  if (Array.isArray(inputOptions.plugins)) {
    ...
    inputOptions.plugins.forEach((plugin) => {
      ...
      if (plugin?.name === "tsconfig-paths") {
        plugins.push(
          chunkIAEJ3C3J_cjs.tsConfigPaths({
            localResolve: true
          })
        );
        return;
    ...
    inputOptions.plugins.push(chunkIAEJ3C3J_cjs.aliasHono());
...
function removeAllOptionsExceptServer(result, logger) {
  return chunkIAEJ3C3J_cjs.removeAllOptionsFromMastraExcept(result, "server", logger);
}
...
//# sourceMappingURL=chunk-Z546LAA6.cjs.map
//# sourceMappingURL=chunk-Z546LAA6.cjs.map